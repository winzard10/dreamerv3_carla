...
REPO_DIR="/scratch/eecs545w26_class_root/eecs545w26_class/brahmsdr/dreamerv3_carla"
CARLA_DIR="/scratch/eecs545w26_class_root/eecs545w26_class/brahmsdr/CARLA_0.9.15"
PORT=2000
...
cd "$CARLA_DIR"
./CarlaUE4.sh -RenderOffScreen -quality-level=Low -nosound -carla-port=${PORT} \
  > "$REPO_DIR/logs/carla-${SLURM_JOB_ID}.log" 2>&1 &
CARLA_PID=$!
echo "Started CARLA PID=$CARLA_PID"

# Always cleanup CARLA on exit/cancel/error
cleanup() {
  echo "Cleaning up CARLA (PID=$CARLA_PID)..."
...
PY
  if [ $? -eq 0 ]; then
    echo "CARLA is ready on 127.0.0.1:${PORT}"
    READY=1
    break
  fi
